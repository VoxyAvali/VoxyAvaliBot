...
def twitchConnection():
    try:
        ...
        print(f"Sending test message to {CHANNEL}´s Chat...")
        sock.send(f"PRIVMSG #{CHANNEL} :{GreetMessage}\r\n".encode("utf-8"))
        print("available console commands: say <message>, reloadcommands, exit")

        sock.send(f"JOIN #voxyavalibot\r\n".encode("utf-8"))
        sock.send(f"PRIVMSG #voxyavalibot :{NICK} found in the chat of {CHANNEL}\r\n".encode("utf-8"))
...
def commandHandler(command, username, sock):
    global running
    ...
    if cmd == "!exit":
        if username.lower() == CHANNEL.lower():
            ...
            time.sleep(2)
            running = False
            sock.send(f"PRIVMSG #voxyavalibot :Bot has disconnected from {CHANNEL}\r\n".encode("utf-8"))
            sock.close()
        else:
                ...
                if "PRIVMSG" in response:
                    ...
                    if len(parts) >= 3:
                        ...
                        if  message.startswith("!"):
                            reply_text = commandHandler(message.lower(), username, sock)
                            if reply_text:
...
if __name__ == "__main__":
    commandLoading()
    sock = twitchConnection()
    if sock:
        input_thread = threading.Thread(target=consoleInput, args=(sock,))
